...
export default function BacklogUpdateBook() {
  ...
  async function grabBacklogBook(e) {
    e?.preventDefault();
    setTitle("");
    setAuthor("");
    setPrimaryGenre("");
    ...
    if (response.ok) {
      ...
      setSeries_name("Unknown");//because the series is not really set
      setSeries_number(book.series_number);
      setTags(book.tag_list);
      setGenres([]);
      setQrs(book.all_qrs);
      // await getCoverThumbnail(isbn);were not going to do this
